...
preprocess_model_path = r"./preprocess_model"

def image2text(meta_file_path, image_path):

    def loading_model():
        local_path = os.path.join(preprocess_model_path, "image_to_text")
        print()
    ...
    for i in tqdm(range(len(pic_name_list))):
        ...
        path = os.path.join(image_path, f"{current_id}")
...
def image2vec(meta_file_path, image_path):
    ...
    def load_VIT_model():
        # processor = ViTImageProcessor.from_pretrained(os.path.join(preprocess_model_path, "vit-base-patch16-224-in21k"))
        # model = ViTModel.from_pretrained(os.path.join(preprocess_model_path, "vit-base-patch16-224-in21k")).to("cuda")
        local_path = os.path.join(preprocess_model_path, "image_to_vec")
        processor = ViTImageProcessor.from_pretrained(local_path)
        model = ViTModel.from_pretrained(local_path).to("cuda")
        return processor, model
    ...
    for i in tqdm(range(0, len(pic_name_list), batch_size)):
        ...
        for path in batch_paths:
            ...
            current_path = os.path.join(image_path, f"{path}")
...
def merged_text_create_and_to_vec(meta_file_path):
    ...
    def load_bert_model():
        # angel = AnglE.from_pretrained(os.path.join(preprocess_model_path, "angle-bert-base-uncased-nli-en-v1"), pooling_strategy='cls_avg').cuda()
        local_path = os.path.join(preprocess_model_path, "text_to_vec")
        angel = AnglE.from_pretrained(local_path, pooling_strategy='cls_avg').cuda()        
        return angel
...
if __name__ == '__main__':
    ...
    image_path = r'./datasets/Instagram/image'
